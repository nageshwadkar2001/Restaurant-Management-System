...
@CrossOrigin(origins="*")  //insted we can directly use specific port like react default port 3000 as well
@RestController              //handles http request and response to given request in the form of JSON
@RequestMapping("/api/v1/")

public class CustomerController {
	
	@Autowired  //no need to int
...
//		return HungerBuzzResponse.success(customerDto);
    ...
    public ResponseEntity<AnnapurnaResponse> forgotPassword(@RequestBody String email) {
        ...
        String subject = "Reset your password for Annapurna";
	...
	/*
	 * @PostMapping("/customers/forgot-password") public
	 * ResponseEntity<AnnapurnaResponse> forgotPassword(@RequestBody String email) {
	 * // Check if the email exists in the database CustomerDto customerDto =
	 * customerService.findCustomerByEmail(email); if (customerDto == null) { return
	 * AnnapurnaResponse.error("Could not find customer with that email address.");
	 * }
	 * 
	 * 
	 * 
	 * 
	 * 
	 * // Generate a password reset token and save it to the database String token =
	 * UUID.randomUUID().toString();
	 * customerService.savePasswordResetToken(customerDto.getId(), token);
	 * 
	 * // Send an email to the customer with a link to reset their password String
	 * recipient = customerDto.getEmail(); String subject =
	 * "Reset your password for HungerBuzz"; String body =
	 * "Please click the following link to reset your password: http://localhost:3000/reset-password/"
	 * + token; EmailService.sendEmail(recipient, subject, body);
	 * 
	 * return AnnapurnaResponse.
	 * success("Password reset token generated and sent to customer."); }
	 */
